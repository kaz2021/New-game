...
var isstart = true;
export class MyGame2 extends Phaser.Scene {
  constructor() {
    super('MyGame2');
  }
  preload() {
    ...
    this.load.tilemapTiledJSON("gamemap2", gameMap2);
  ...
  create(passingdata) {
    ...
    const map = this.make.tilemap({ key: "gamemap2" });
    ...
    this.player = this.physics.add.sprite(1028, 640, "player-run");
  ...
  update() {
    ...
    if (checkOverlap4(this.player.x, this.player.y, 1024, 1400) == true) {
      if (redcircleobject != null) {
        //redcircleobject.destroy(true);
      }
      clearTimeout(this.treasureboxtimer);
      clearTimeout(this.gametimer);
      clearTimeout(redcircletimer.timer);
      clearInterval(this.pointtimer);
      var passingdata = { isPlay: true, score: this.point };
      this.scene.start("MyGame", passingdata);
    }
  }
